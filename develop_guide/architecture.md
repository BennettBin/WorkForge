...
- Time: 2026-05-10 16:20
  Change Summary: Reorganized skill runtime architecture to skill-package layout and dynamic runtime binding. Each skill is now stored at `main/backend/app/skills/<domain>/<skill_name>/` and contains `SKILL.md` + `runtime.py`. `SkillRegistry` now scans `SKILL.md` in skill folders; `SkillExecutor` now resolves `runtime_handler` and loads callable from skill-local runtime file instead of hardcoded `if/else`.
  Impact Scope: `main/backend/app/skills/**/SKILL.md`, `main/backend/app/skills/**/runtime.py`, `main/backend/app/services/skill_registry/registry.py`, `main/backend/app/services/skill_runtime/executor.py`.
  Notes: Task orchestration keeps using `TaskService._select_skills(...)` + `SkillExecutor.execute(...)`, but effective execution source is now `app/skills` runtime packages, matching skill-runtime enforcement and auditability requirements.
- Time: 2026-05-10 17:05
  Change Summary: Migrated all task sub-agent implementation scripts from `main/backend/app/agents/sub_agents` into `main/backend/app/skills/<domain>/sub_agents` and removed old `agents/sub_agents` directory. Task agents and task manager imports were switched to skill-domain packages under `app.skills.*`, so task execution code now resides directly in `app/skills`.
  Impact Scope: `main/backend/app/skills/{ppt,report,wechat_post,data_analysis,code_doc,paper_assistant}/sub_agents/*`, `main/backend/app/agents/task_agents/*.py`, `main/backend/app/services/task_manager/task_service.py`, deletion of `main/backend/app/agents/sub_agents/`.
  Notes: Skill runtime discovery/execution remains `SkillRegistry + SkillExecutor`; this migration additionally colocates task execution scripts with skill assets in the same top-level `skills` tree.
