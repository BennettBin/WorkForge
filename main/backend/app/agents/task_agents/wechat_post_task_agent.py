    ...
    def execute(
        ...
        try:
            finder = skill_execute_fn(
                "find_skill",
                {"task_type": "wechat_post", "requirement": requirement, "preferred_skills": ["wechat_post_generation"]},
            )
        except Exception:
        ...
        if not matched:
            return self._direct_fallback(requirement, parsed_text, llm_generate_fn)
        skill_name = matched[0] if len(matched) > 0 else "wechat_post_generation"
        try:
        ...
        return WechatPostTaskArtifacts(
            ...
            plan_summary=str(result.get("plan_summary", "wechat_post plan generated")),
