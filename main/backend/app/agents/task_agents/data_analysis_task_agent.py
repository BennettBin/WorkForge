...
class DataAnalysisTaskAgent:
    def execute(
        ...
        plan = skill_execute_fn("data_analysis_planner", {"requirement": requirement, "parsed_text": parsed_text, "language": language})
        draft = skill_execute_fn(
            "data_analysis_writer",
            {
        ...
        review = skill_execute_fn("data_analysis_reviewer", {"draft": draft})
        return DataAnalysisTaskArtifacts(
            ...
            plan_summary=str(plan.get("summary", "data analysis plan generated")),
